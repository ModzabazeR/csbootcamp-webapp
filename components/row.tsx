...
const Row: React.FC<{ groupUser: IUser }> = ({ groupUser }) => {
  ...
  const [oldScore, setoldScore] = useState(groupUser.point);
  ...
  const handleChange = (event: any, groupUser: IUser) => {
    ...
    groupUser.point = oldScore + Number(event.target.value);
  };

  return (
    <div className="block w-full text-center" key={groupUser.id}>
      <div className="grid grid-cols-3 gap-4 items-center">
        <div className="text-center pr-10 pl-5">{`${getGroupName(groupUser.id)} (${groupUser.id})`}</div>
        <div className="border-x-2 px-5 border-x-black">
          <div className="relative grid grid-cols-2 gap-4 items-center">
            ...
            <input
              ...
              type="text"
              id="score"
              className="my-2 block w-full py-4 px-10 text-center dark:focus:ring-blue-500 dark:focus:border-blue-500 rounded-lg"
              placeholder="Score"
